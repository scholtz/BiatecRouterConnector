...
- **Line endings are LF everywhere** (`.editorconfig`'s `end_of_line = lf`, `.gitattributes`'
  `* text=auto eol=lf`). This was a deliberate fix: CI runs on Linux, where git checks files out
  with LF regardless of what a Windows dev's `core.autocrlf` produces locally, and `dotnet format`
  enforces whatever `.editorconfig` says — CRLF and LF disagreeing between the two caused
  `ENDOFLINE` failures on every tracked `.cs` file. If you're on Windows and `git config
  core.autocrlf` is `true`, your working tree can still show CRLF after a normal checkout even
  though `.gitattributes` says `eol=lf` (a real, observed git quirk, not a config mistake) — don't
  "fix" this by reverting to `crlf` in `.editorconfig`. Auto-generated files (anything with an
  `<auto-generated>` header, e.g. `obj/BiatecRouterApiClient.cs`) are exempt — `dotnet format` skips
  them regardless of line endings.
